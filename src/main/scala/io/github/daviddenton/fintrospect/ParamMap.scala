...
import com.twitter.finagle.http.util.StringUtil.toSomeLong
import com.twitter.finagle.http.{MediaType, Method}
import com.twitter.io.Charsets
import io.github.daviddenton.fintrospect.FinagleTypeAliases._
import org.jboss.netty.handler.codec.http.{HttpHeaders, QueryStringDecoder, QueryStringEncoder}
...
object MapParamMap {
  ...
  def tuplesToMultiMap(
                        tuples: Seq[Tuple2[String, String]]
                        ): Map[String, Seq[String]] = {
    tuples
      .groupBy { case (k, v) => k}
      .mapValues { case values => values.map {
      _._2
    }
    }
  }
...
class RequestParamMap(val request: FTRequest) extends ParamMap {

  /** Get Content-Type header */
  def contentType: Option[String] = Option(request.headers().get(HttpHeaders.Names.CONTENT_TYPE))
  ...
  private[this] val postParams: JMap[String, JList[String]] = {
    if (request.getMethod != Method.Trace &&
      mediaType(contentType) == Some(MediaType.WwwForm) &&
...
object ParamMap {
  ...
  val EmptyJMap = new java.util.HashMap[String, JList[String]]
}
