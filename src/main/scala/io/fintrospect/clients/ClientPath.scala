...
case class ClientPath private(client: ClientRoute, method: HttpMethod, pathParametersInReverse: Seq[PathParameter[_]]) {
  ...
  def bindTo(service: Service[HttpRequest, HttpResponse]) = new Client(method, client.headerParams, client.queryParams, pathParametersInReverse.reverse, client.body, service)
}
