...
case class ParameterSpec[T](name: String,
  ...
  def apply[O <: ParameterSpecVal[T]](fn: T => O) =
    ParameterSpec[O](name, description, paramType, s => fn(deserialize(s)), b => serialize(b.value))
}

/**
  * Convenience trait for which ParameterSpecs can be autogenerated.
  */
