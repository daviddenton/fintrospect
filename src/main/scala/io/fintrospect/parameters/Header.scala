...
object Header {
  ...
  val required = new Parameters[HeaderParameter, Mandatory] {
    override def apply[T](spec: ParameterSpec[T]) = new HeaderParameter[T](spec) with Mandatory[T]
  }

  val optional = new Parameters[HeaderParameter, Optional] {
    override def apply[T](spec: ParameterSpec[T]) = new HeaderParameter[T](spec) with Optional[T]
  }
}
