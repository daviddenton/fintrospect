...
abstract class SingleHeaderParameter[T](spec: ParameterSpec[T])
  ...
  protected def get(message: Message) = Extraction.extract(this, xs => spec.deserialize(xs.head), extract(message))
...
abstract class MultiHeaderParameter[T](spec: ParameterSpec[T])
  ...
  protected def get(message: Message) = Extraction.extract(this, xs => xs.map(spec.deserialize), extract(message))
