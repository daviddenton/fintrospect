...
class XmlDescriptionRenderer extends DescriptionRenderer[HttpResponse] {
  ...
  def apply(basePath: Path, routes: Seq[Route]): HttpResponse = XmlResponseBuilder.Ok(XmlFormat( s"<paths>${renderRoutes(basePath, routes)}</paths>").value)
}
