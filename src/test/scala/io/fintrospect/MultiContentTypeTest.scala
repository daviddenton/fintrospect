...
class MultiContentTypeTest extends FunSpec with ShouldMatchers {

  describe("MultiContentType") {
    it("on no match, return 406") {
      val r = result(MultiContentType()(requestWithAcceptHeaders(APPLICATION_JSON.value)))
      r.status shouldBe NotAcceptable
    }
    it("when there are no accept header set, just chooses the first type") {
      val r = result(MultiContentType(serviceForType(APPLICATION_ATOM_XML), serviceForType(APPLICATION_JSON))(requestWithAcceptHeaders()))
    ...
    it("when there is an exact (case insensitive) match on a content type, use that") {
      val r = result(MultiContentType(serviceForType(APPLICATION_ATOM_XML), serviceForType(APPLICATION_JSON))(requestWithAcceptHeaders("apPliCation/jSoN")))
      r.status shouldBe Ok
      r.headerMap("Content-Type") should startWith(APPLICATION_JSON.value)
    }

    it("ignores q values and levels") {
      val r = result(MultiContentType(serviceForType(TEXT_HTML), serviceForType(APPLICATION_JSON))
