...
import com.twitter.finagle.httpx.{Method, Request, Status}
      ...
      val module = FintrospectModule(Root / "basepath", renderer)
        .withRoute(
          RouteSpec("summary of this route", "some rambling description of what this thing actually does")
            ...
            .at(Method.Get) / "echo" / Path.string("message") bindTo ((s: String) => Echo(s)))
        .withRoute(
          RouteSpec("a post endpoint")
            ...
            .body(customBody)
            .at(Method.Post) / "echo" / Path.string("message") bindTo ((s: String) => Echo(s)))
        .withRoute(
          RouteSpec("a friendly endpoint")
            ...
            .at(Method.Get) / "welcome" / Path.string("firstName") / fixed("bertrand") / Path.string("secondName") bindTo ((x: String, y: String, z: String) => Echo(x, y, z)))
