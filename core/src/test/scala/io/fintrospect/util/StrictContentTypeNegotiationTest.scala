...
class StrictContentTypeNegotiationTest extends FunSpec with Matchers {
  ...
  describe("StrictContentTypeNegotiation") {
    it("on no match, return 406") {
      val r = result(StrictContentTypeNegotiation.of(serviceForType(APPLICATION_ATOM_XML))(requestWithAcceptHeaders(APPLICATION_JSON.value)))
      r.status shouldBe NotAcceptable
    }

    it("when there are no accept header set, just chooses the first type") {
      val r = result(StrictContentTypeNegotiation.of(serviceForType(APPLICATION_ATOM_XML), serviceForType(APPLICATION_JSON))(requestWithAcceptHeaders()))
    ...
    it("when there is an exact (case insensitive) match on a content type, use that") {
      val r = result(StrictContentTypeNegotiation.of(serviceForType(APPLICATION_ATOM_XML), serviceForType(APPLICATION_JSON))(requestWithAcceptHeaders("apPliCation/jSoN")))
      r.status shouldBe Ok
      r.headerMap("Content-Type") should startWith(APPLICATION_JSON.value)
    }

    it("ignores q values and levels") {
      val r = result(StrictContentTypeNegotiation.of(serviceForType(TEXT_HTML), serviceForType(APPLICATION_JSON))
